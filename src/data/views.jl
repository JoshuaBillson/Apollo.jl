...
Base.length(x::MappedView{<:Any,<:Tuple}) = map(length, data(x)) |> minimum
...
Base.getindex(x::ObsView, i::AbstractVector) = _stack([getindex(x, j) for j in i])
...
```
"""
function splitobs(data; at=0.8, shuffle=true)
